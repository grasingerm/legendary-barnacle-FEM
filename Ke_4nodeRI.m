...
function retval = Ke_4nodeRI(D, x, y, n)
  assert(length(x) == 4 && length(y) == 4, "element nodal coordinates must have a length of 4");
  assert(size(D) == [3 3], "D matrix must be 3 x 3");
  if nargin < 6
    n = 1;
  endif
  [gps, wts] = n_quad_wap(n);
  retval = quad_2D(@(xi, eta) K_integrand(D, x, y, dN_4node(xi, eta)), gps, wts);
endfunction
